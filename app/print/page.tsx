...
export default function PrintPage() {
  ...
  return (
    <main className="min-h-screen bg-white text-[#17211c]">
      <div className="print:hidden flex items-center justify-between gap-4 border-b border-black/10 bg-white px-4 py-3 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2">
          <Button
            ...
            variant={orientation === "landscape" ? "default" : "secondary"}
          >
            Landscape
          </Button>
          <Button
            onClick={() => setOrientation("portrait")}
            variant={orientation === "portrait" ? "default" : "secondary"}
          >
            Portrait
          </Button>
      ...
      <section className={`printable-page ${orientation} p-5 print:p-0`}>
        <header>
          <div>
            <h1>{sheet.title}</h1>
...
function PrintableSheetStyles({ orientation }: { orientation: "portrait" | "landscape" }) {
  return (
    <style>{`
      @page { margin: 8mm; size: letter ${orientation}; }
      .printable-page { --cell: 48px; background: #fff; color: #17211c; font-family: Arial, "Noto Sans TC", "Microsoft JhengHei", sans-serif; margin: 0 auto; transition: max-width 160ms ease; }
      .printable-page.landscape { max-width: none; }
      .printable-page.portrait { --cell: 46px; max-width: 900px; }
      ...
      @media print {
        ...
        .printable-page { max-width: none !important; padding: 0 !important; transition: none !important; }
      }
    `}</style>
  );
}
